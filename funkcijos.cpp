...
void NewStudent(std::vector <Studentas>& studentas) {			//Studento duomenu ivedimas (manualinis)				
	bool pazym, sk;
	int z = 0;
...
void ivesk(T& studentas, std::string fileName, char rc, bool timeOut)
{
	...
	try
	{
		...
		while (getline(file, line))
		{
			...
			stud.setFinal(skaiciuok(stud.getPazymiai(), stud.getEgzas(), stud.getN(), rc));
			studentas.push_back(stud);
		}
	}
	catch (int e)
